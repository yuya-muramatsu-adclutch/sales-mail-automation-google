...
- Web app @362 / production code v352: `https://script.google.com/macros/s/AKfycbwJcZuTk-7wuFJapBdo4dk-yj64hFHk71BMuJxO-pl9BWpui3kOt17lmPT_7LfnZ0OV-g/exec`
- Spreadsheet DB: `https://docs.google.com/spreadsheets/d/1IuJrWB7RGd2qIFDlhe5lfKaBnmUKN4RcnxdFFTuluZY/edit`
- Apps Script HEAD / repository code: `20260818_apps_script_full_workflow_v352_mail_status_next_send_first`

## v352 メール送信状況を「次回送信」中心に再設計

- 原因は、従来画面が自動送信、明日の候補、優先ジャンル、要確認を同格の4カードで並べ、その下で明日の候補と優先設定を別パネルに分けていたことだった。ユーザーが最初に知りたい「次に何件送るか」と「グランピング以外へ送られないか」が分散し、カード・余白・枠線が多く視線の順序も不明確だった。
- 選択されたデザイン案1に合わせ、稼働・次回確認・安全状態・停止操作を上部の1本の運用バーへ集約した。明日の予定日と候補件数を大きく表示し、1日上限、1回上限、進捗、送信時間帯を同じ次回送信領域へまとめた。
- グランピング優先を横長の状態バーへ変更し、対象件数、未送信・通常送信済み内訳、他ジャンル停止、テンプレート、対象内訳、LP、解除操作を明日の候補表の直前へ配置した。正常時の説明アラートは非表示にし、失敗・送信中・設定不備など対応が必要な警告だけを表示する。
- 明日の候補表と今日の送信履歴は既存のAPI、絞り込み、施設詳細導線を維持した。自動送信、優先設定、送信条件、日次・バッチ上限、二重送信防止などサーバー処理は変更していない。
- 1180px以下では運用バーを2列、760px以下では1列へ変更する。次回送信と優先設定も縦並びにし、候補・履歴テーブルは既存の横スクロールを維持した。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `362`を作成し、固定Web appを`@362`へ更新した。実メール送信、設定変更、本番営業データ変更は行っていない。
- 選択された参考画像は確認済みだが、Browserの画面取得・操作ランタイムがこのセッションに公開されていないため、ログイン済み本番画面の同一画角比較は未実施。静的・回帰検証を実画面の目視合格とは扱っていない。

## v351 ボタン操作とフォーム候補表示の体感高速化

- コード上、`showTab`が取得済み画面への移動でも毎回650msのルート進捗を表示していた。また、`api()`が呼び出し直後に`setBusy(true)`を実行し、フォーム送信済み・送信不可・候補読込のように局所更新できる操作でも、アクティブ画面の全ボタンを無効化して上部進捗を表示していた。ボタンを押すたびにローディングが見える直接原因と特定した。
- 共通APIのbusy表示を450ms遅延し、完了が速い処理では表示しない。1.2秒以上の処理はブラウザconsoleへ処理名と経過時間を記録する。メニュー進捗は未取得データがある場合だけ220ms後に表示し、実際の画面データ取得完了に連動して消す。
- フォーム候補読込、送信済み、送信不可、送信済み解除を`apiQuiet`へ変更し、既存のカード内pending状態とローカルキュー更新だけを使う。通常の状態変更もサーバー返却レコードを反映し、フォーム一覧と営業リストの再取得を行わない。
- 本番Spreadsheetメタデータを読み取り専用で再確認し、`leads`は10,153行・46列・ヘッダー1行で、候補初回計算が10,152件規模を走査する構造だった。同一検索・ジャンル条件をブラウザで60秒、サーバーで5分キャッシュし、同じ操作による再走査を避ける。営業先、テンプレート、送信履歴、送信NG・除外設定の更新時はcache revisionを更新し、古い候補を失効させる。
- フォーム送信メニューのpointer/focus時にテンプレートと候補を先読みし、画面表示時は同じin-flight処理またはキャッシュを再利用する。キャッシュ破棄中に古いリクエストが完了しても再登録しないrevision guardを追加した。
- `clasp logs --json`はGCP project ID未設定のため利用できず、本番実行ログの所要時間比較は未取得。代わりにv351以降は1.2秒超のブラウザAPI時間をconsoleへ記録する。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `361`を作成し、固定Web appを`@361`へ更新した。実メール送信、外部フォーム送信、送信済み・送信不可登録など本番営業データを変更する検証は行っていない。
- Browserの画面取得・操作ランタイムがこのセッションに公開されていないため、ログイン済み本番画面でのクリック計測と画像比較は未実施。静的・回帰検証を実画面の目視合格とは扱っていない。

## v350 フォーム送信の連続作業キュー

- 原因は、従来のフォーム送信画面が全候補テーブル、固定作業バー、本文プレビューを同時表示し、施設選択、公式サイト移動、本文コピー、状態更新の操作が分散していたことだった。さらに、送信済み操作後の再読込が作業位置を変え、連続処理を妨げていた。
- サーバー側に`listFormOutreachQueue`を追加し、本番フォームテンプレート設定済み、公式サイトあり、送信条件を満たす施設だけを最大25件の作業キューへ返す。テンプレート不足、送信NG、公式サイト不足、待機期間中、その他の遮断理由は`reviewItems`と理由別集計へ分離した。
- 画面を「現在の1施設」と「次の5件」の集中型へ変更した。公式サイトのトップを開いて本文コピー、送信済み・次へ、送信不可・次へを主要操作とし、PC用の`O`、`S`、`X`ショートカットを追加した。本文はテンプレート名と冒頭2行を表示し、全文は必要時だけ展開する。
- 送信済みと送信不可は確認画面を出さず、成功した施設をローカルキューから除いて次へ進む。25件を処理し終えた時だけ次の候補を取得するため、1件ごとの一覧再読込を行わない。送信不可は`status`と`form_status`を対応不要へ変更し、理由`フォームが見つからない`、時刻、変更前状態をカスタム項目へ保存する。
- 本番Spreadsheetを読取専用で確認し、グランピングのジャンル行743件、メールなし・フォームURLあり・公式サイトありの生データ候補130件、専用フォームキャンペーン記録4件を確認した。キューではこれらへテンプレート、状態、重複送信、30日待機などの既存条件を追加適用する。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `359`を作成し、固定Web appを`@359`へ更新した。実メール送信、外部フォーム送信、送信済み登録、送信不可登録など本番営業データを変更する検証は行っていない。
- 参考デザインに基づくDOM、レスポンシブCSS、操作経路の静的検証は完了した。ログイン済みBrowserを開くことはできたが、このセッションには画面取得・操作ランタイムが公開されていないため、デプロイ後の同一画角スクリーンショット比較は未実施であり、目視合格とは扱っていない。

## v349 WEBサイトアイコンの巨大化修正

- 本番スクリーンショットで、フォーム対象の各行に外部リンクアイコンが約200px超で表示され、行高と施設名表示を壊していた。`legacyUiIcon('external')`がwidth/heightを持たず、v348で追加したアンカー要素には既存のbutton用SVG寸法ルールが適用されないことを原因と特定した。
- `.form-facility-name-link svg`を14px、`.form-work-title-link svg`を15px、`.url-mini-link > svg`を14pxに固定し、`stroke-width: 2.15`で既存アイコンと揃えた。WEBサイトリンク、屋号コピー、作業選択の挙動は変更していない。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `358`を作成し、固定Web appを`@358`へ更新した。実メール送信、外部フォーム送信、送信済み登録、営業先データ更新は行っていない。

## v348 フォーム送信リストのWEBサイト導線

- 添付画面では表の左側に施設名、ジャンル、テンプレートだけが見え、WEBサイト列は右側の横スクロール外だった。選択施設「割山森林公園 天湖森」を本番読取APIで確認し、`website_url=https://tenkomori.info/`と`form_url=https://tenkomori.info/contact`が保存済みだったため、データ不足ではなく情報配置を原因と特定した。
- WEBサイト列を表の後半から撤去し、「営業先・WEBサイト」セルへ統合した。施設名そのものと明示的な「WEBサイトを開く」の2か所を新規タブの公式サイトリンクとし、屋号コピーと作業選択は別ボタンへ分離した。
- 作業中バーでは公式サイトを最初の青い主要ボタンへ変更し、施設名も公式サイトリンクにした。既存のフォーム表示、本文コピー、送信済み、次への操作順と状態判定は維持した。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `357`を作成し、固定Web appを`@357`へ更新した。実メール送信、外部フォーム送信、送信済み登録、営業先データ更新は行っていない。

## v347 フォーム送信作業の操作性改善

- フォーム送信一覧に営業先WEBサイトへの直接リンクを追加し、作業中バーにも「WEBサイトを開く」を配置した。屋号・本文コピーは操作結果だけを部分更新し、一覧全体を再描画しない。
- グランピング向けChatGPT広告フォーム本文の先頭を`屋号`、改行、`ご担当者様`へ変更した。保存済みテンプレートが旧versionでも、組み込みversion 2を一覧取得と送信済み記録の双方へ適用する。
- 明示的な一覧更新では既存行を消さずに「更新中…」を表示する。送信済みチェックと解除は、従来の`loadFormLeads()`と`loadLeads()`による全件再取得を廃止し、サーバー返却レコードで該当行、集計、作業中パネルだけを更新する。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Scriptの旧未使用version 175〜329が削除され、保存version数が200件から45件へ減ったことと、既存21デプロイが維持されていることを確認した。
- Apps Script version `356`を作成し、固定Web appを`@356`へ更新した。外部フォーム送信、フォーム送信済み登録、実メール送信、営業先データ更新は検証で行っていない。

## v346 フォーム用ChatGPT広告テンプレートのクイックコピー

- フォーム送信画面には表内と本文プレビューにコピー操作があったが、ChatGPT広告専用テンプレートをすぐ使うための入口が小さく、選択中の施設向け本文とテンプレート原文の違いも判断しづらかった。
- 画面上部に専用コピーエリアを追加し、表示中の送信可能なグランピング施設へ屋号を差し込んだ本文と、専用テンプレート原文を別々に1クリックでコピーできるようにした。コピー後はボタン表示を「コピーしました」へ一時的に切り替える。
- 表内と作業中バー、本文プレビューでも、専用テンプレート利用時は「ChatGPT広告をコピー」と表示する。既存のジャンル絞り込みへ移動する「グランピングを表示」も同じエリアへ配置した。
- コピー処理はブラウザのクリップボードだけを更新し、外部フォーム送信、フォーム送信済み登録、営業先データ更新は行わない。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `355`を作成し、固定Web appを`@355`へ更新した。`clasp clone`で取得した`@355`の9ファイルとローカル送信対象が一致することも確認した。ブラウザ操作ランタイムが利用できず、ログイン済み本番画面の最終目視は未実施。

## v345 フォーム送信リストのジャンル絞り込み

- フォーム送信画面の既存フィルターは検索、フォーム状態、表示件数だけで、営業リスト側では利用できるジャンル条件をフォーム一覧から指定できなかった。
- ジャンルマスターからフォーム用ジャンル選択肢を生成し、選択変更時に一覧を再取得するようにした。マスター再読込後も有効な選択値を維持し、クリア操作では「すべて」へ戻す。
- 選択ジャンルは`listLeads({ filter: 'form_all', genre: ... })`へ渡すため、表示後のクライアント絞り込みではなく、総件数と取得行の両方へ同じ条件が適用される。
- 実メール送信、外部フォーム送信、フォーム送信済み登録、営業先データ更新は行っていない。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `354`を作成し、固定Web appを`@354`へ更新した。`clasp clone`で取得した`@354`の9ファイルとローカル送信対象が一致することも確認した。

## v344 メール対象施設のフォーム一覧重複除外

- コード上、同じ行の`email`が有効な場合は従来からフォーム一覧の対象外だった。本番`leads` 10,152件を読み取り確認した結果、残り得る原因は同じ施設がメール付き行とフォームだけの別行に分かれている重複レコードだった。
- 同一ジャンル・正規化施設名が一致する有効メール付きレコードを一覧取得時に索引化し、対応するフォーム専用レコードを`form_all`から除外するよう修正した。本番読み取り確認では2組がこの構造に該当した。
- ドメイン単独一致は使わない。本番データでは共有サイト上の異なる3施設が同じドメインを使っており、ドメイン一致だけで除外すると誤除外になることを確認した。
- 実メール送信、外部フォーム送信、フォーム送信済み登録、営業先データ更新は行っていない。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `353`を作成し、固定Web appを`@353`へ更新した。`clasp clone`で取得した`@353`の9ファイルとローカル送信対象が一致することも確認した。

## v343 グランピング向けChatGPT広告のフォーム営業対応

- フォーム画面は従来、`template_type=form`の通常テンプレートだけをジャンル一致で選んでおり、メール用のChatGPT広告テンプレートは利用できなかった。また、フォーム送信履歴にはキャンペーン識別子がなく、メールとフォームをまたぐ同一広告の重複判定もできなかった。
- `glamping-chatgpt-ads-form-v1`をフォーム専用テンプレートとして追加し、グランピング施設へ自動選択する一方、同ジャンルまたは共通の通常フォームテンプレートへ施設単位で切り替えられるようにした。専用文面はLPを含み、メール版より短い。
- 通常フォーム送信から30日以上経過した施設を再び対象に含める。専用メール成功・送信中、専用フォーム送信済み、30日未満、送信日不明、確認待ち、送信NG、商談中などはサーバー側で止め、画面に理由を返す。
- メール予約とフォーム送信済み登録を同じスクリプトロックで直列化し、メール予約が先ならフォームを、フォーム記録が先ならメールを停止する。専用フォーム本文はサーバー側テンプレートから生成し、ブラウザから渡された本文へ差し替えない。
- 専用フォーム送信を解除した場合はキャンペーン印だけを外し、それ以前の通常フォーム送信イベント、送信回数、最新通常フォーム日を復元する。
- 実メール送信、外部フォーム送信、フォーム送信済み登録、営業先更新は行っていない。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `352`を作成し、固定Web appを`@352`へ更新した。`clasp clone`で取得した`@352`の9ファイルとローカル送信対象が一致することも確認した。

## v342 メール送信状況の判断優先レイアウト

- 原因はデータ不足ではなく配置だった。従来は大きな状態カード2枚とKPI6枚の後に今日の履歴があり、明日の候補はさらに下にあった。グランピングが0件になる理由も、別の診断パネルを開くまで件数が見えなかった。
- 上部を「自動送信」「明日の候補」「優先ジャンル」「要確認」の4カードへ整理した。明日の候補を左、グランピング優先設定を右へ並べ、今日の履歴を下段へ移動した。
- 優先設定カードに送信可能件数、未送信・通常送信済みの内訳、30日ルール、専用メール送信済み除外、開始・解除・対象内訳の操作をまとめた。候補0件時は対象外理由を上位4項目まで件数カードで表示し、全施設の診断へ移動できる。
- 既存の候補算出、送信条件、管理者制限、1日上限、1回上限、二重送信防止は変更していない。実メール送信、優先設定変更、営業先更新は行っていない。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `351`を作成し、固定Web appを`@351`へ更新した。
- 自動ブラウザ接続が利用できず、ログイン済み本番画面の同一画角比較は未実施。静的検証を目視確認済みとして扱っていない。

## v341 返信同期とダッシュボード送信候補の整合

- `recordDetectedReply_` が返信検知時に常にステータスを「返信あり」へ更新し、既存の送信NGを解除し得ることをコードで確認した。送信NGフラグまたは「送信NG」ステータスがある場合は、その状態を保持して `reply_checked` と Gmail thread ID だけを反映するよう修正した。
- ダッシュボードの `todayEmailTargets` が汎用の `listStats.sendable`を使い、テンプレートとジャンルが合わない候補も数えていた。送信制御、トリガー、送信時間帯の有効性、滞留予約、本番テンプレート、ジャンル、グランピング優先、送信履歴と残上限を同じ候補選択関数で評価するよう修正した。
- `gmail_reply_check` を `{"enabled":true,"maxThreads":200}` へ変更した。Gmailの送信日以降の受信を照合し、人からの返信と確認できた3件を `reply_logs` と営業先の `reply_checked` / `last_gmail_thread_id` へ同期した。同期後に3件とも `status=送信NG`、`send_ng=TRUE`、`reply_checked=TRUE` であることを再読込みで確認した。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`が成功した。Apps Script version `350`を作成し、固定Web appを`@350`へ更新した。Google側version 350を再取得し、9ファイルすべてがローカルと一致した。
- 実メール送信は行っていない。画面操作用のブラウザ接続機能が利用できず、ログイン済み本番画面の最終目視は未実施。
- GitHub反映は、作業環境に `gh` が導入されていないため未実施。本番コードとローカル差分は保持し、認証後に `codex/email-delivery-status` を公開する。

## v340 グランピング専用メールの30日再送対象

- 通常メールを送信済みのグランピング施設も、前回の成功送信から30日以上経過していればChatGPT広告専用メールの対象にした。未送信施設を先に並べ、その後は前回送信が古い施設から送信する。
- 専用テンプレートID `glamping-chatgpt-ads-v1` の成功履歴を営業先ID・宛先の両方で照合し、専用メール送信済み施設は再送対象外にした。送信履歴の種別は `グランピングChatGPT広告` として通常メールと区別する。
- 候補一覧、手動一括送信候補、優先開始判定、自動送信、送信直前の二重送信防止を同じ判定へ統一した。診断画面には未送信、通常メール送信済み、30日待機中、専用メール送信済みなどの理由を表示する。
- 本番DBを読み取り専用で照合した結果、グランピング743件のうち現時点の送信可能候補は333件（未送信0件、通常メール送信済み333件）。対象外はメールアドレスなし・不正359件、営業対象外19件、送信NG10件、確認待ち8件、除外ドメイン9件、30日待機中5件。専用メールの本番成功履歴は0件だった。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `349`を作成し、固定Web appを`@349`へ更新した。Google側version 349を再取得し、ローカルの`Code.gs`・`Email.gs`・`Index.html`と一致することを確認した。
- 実メール送信、優先モード開始、営業先・テンプレート・設定の変更は行っていない。ログイン済みブラウザを操作できるランタイムが利用できなかったため、本番画面の最終目視は未実施。

## v339 グランピング優先の対象・除外理由表示

- 本番DBを読み取り専用で再確認し、ジャンル名にグランピングを含む営業先743件のうち、現在の送信可否判定を通る対象が0件であることを確認した。内訳はメールアドレスなし・不正359件、送信済み346件、営業対象外19件、送信NG10件、確認待ち8件、除外ドメイン1件だった。
- 原因は、画面が`targetCount > 0`のときだけ優先開始ボタンを有効化し、0件時の施設一覧・除外理由をAPIで返していなかったこと。対象が0件でもボタンを「開始できない理由を確認」として有効にし、設定変更を行わず判定一覧を開くよう修正した。
- サーバー側で既存の`getEmailSendTargetBlockReason_`を再利用し、グランピング全件を理由別に集計する診断データを追加した。画面では全件・理由別件数、施設名、メールアドレス、現在状態、判定、詳細理由を表示し、理由フィルターと営業先詳細への導線を追加した。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `348`を作成し、固定Web appを`@348`へ更新した。Google側version 348を再取得し、v339版名・診断API・理由確認ボタン・診断一覧要素を確認した。
- 実メール送信、営業先更新、テンプレート更新、設定変更は行っていない。ログイン済み本番画面の最終目視は未実施。

## v338 メール送信状況の余白・テンプレート待ち表示修正

- ユーザー提供の本番スクリーンショットで、送信状況パネルの見出しと説明がカード端へ寄り、見出しから表までの余白も不均一な状態を確認した。対象パネルに既存の`panel stack`余白規則を適用し、見出し内の段落marginをリセットして、カード・説明・表を同じ間隔へ揃えた。
- 本番Spreadsheetを読み取り専用で監査した。営業先10,152件、送信履歴1,440件、除外ドメイン697件を既存の送信可否判定と同じ条件で照合した結果、固有宛先ベースの送信可能対象は1,953件だった。内訳はECサイト1,922件、介護施設29件、ジャンル未設定2件で、本番ONの初回メールテンプレート3種（グランピング・温泉旅館・キャンプ）と一致する対象は0件だった。
- 送信可能対象が存在するのに本番テンプレートがない場合、明日の実送信候補を偽って作らず、「本番テンプレート設定待ち」として実データを最大1日上限件数まで表示するようにした。ジャンル別件数、施設、宛先、送信されない理由、テンプレート設定画面への導線を表示する。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `347`を作成し、固定Web appを`@347`へ更新した。Google側のversion 347ソースを再取得し、v338版名・テンプレート待ち集計・余白クラス・表示要素を確認した。
- 実メール送信、営業先更新、テンプレート更新、設定変更は行っていない。`clasp run getEmailDeliveryStatus`は実行権限不足で利用できず、ログイン済み本番画面の最終目視は未実施。

## v337 メール送信状況の読み込み・レイアウト修正

- ユーザー提供の本番スクリーンショットで、初回取得中に上部カードが左列だけへ表示され、今日・明日の一覧が空のままになる状態を確認した。コード上では`getEmailDeliveryStatus`が約10,153件の営業先を読み込み、候補選定と送信回数監査を終えるまで画面へ一切返さない構造だった。
- 今日の履歴・自動送信・時間帯・優先テンプレートの状態だけを先に返す`getEmailDeliveryOverview`を追加し、明日の候補は独立して取得するよう変更した。候補計算中は0件と表示せず、失敗時は理由と候補だけの再取得ボタンを表示する。
- 明日の候補計算は、全候補を1回選定したあと、実処理と同じ1回上限ごとのジャンル・テンプレート順へ再構成する方式に変更した。グランピング優先中は対象件数の選定結果を候補計算にも再利用する。
- 読み込みカードを2列グリッド全幅へ修正し、送信状況画面の最大幅を既存デザイントークンと同じ1680pxに揃えた。PCを主表示とし、820px以下のカード縦積み、760px以下の一覧横スクロールは維持した。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `346`を作成し、固定Web appを`@346`へ更新した。Google側のversion 346ソースを再取得し、v337版名・軽量API・段階取得・全幅待機カードを確認した。
- メール送信、営業先更新、設定変更は行っていない。ログイン済みブラウザを操作できるランタイムがないため、本番画面の最終目視はユーザー側での再読み込み確認が必要。

## v336 メール送信状況ボード

- 本番DBを読み取り専用で確認し、`mail_sending_control.enabled=true`、送信時間`07:00-08:00 / Asia/Tokyo`、日次上限80件、1回上限20件、グランピング優先OFFを確認した。2026年8月15日の履歴はグランピング専用テンプレートの成功したテスト送信2件で、本番営業先への送信履歴はなかった。
- 既存の送信可否判定と`selectScheduledEmailCandidates_` / `selectEmailGenrePriorityCandidates_`を再利用する読み取り専用API `getEmailDeliveryStatus`を追加した。明日の候補は1回上限ごとに安全コンテキストへ仮予約し、実処理と同じテンプレートグループ順で日次上限まで並べる。
- 主要メニューへ独立した「メール送信状況」を追加した。上部に自動送信、送信時間、次回送信、本日の残り、優先ジャンル、テンプレート、警告を集約し、今日の状態別履歴と明日の候補を同じ画面に表示する。
- 自動送信の開始・停止とグランピング優先の開始・解除は管理者だけに限定した。優先開始前は対象件数、専用テンプレート、他ジャンルを停止することを確認し、操作直後に画面を再取得する。
- 画面表示時と日本時間6:30・12:00・18:00に更新し、PCを主表示、760px以下ではカードを縦積み、一覧を横スクロールにした。失敗・送信中には再送ボタンを置かず詳細確認だけにした。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `345`を作成し、固定Web appを`@345`へ更新した。Google側のversion 345ソースを再取得し、v336版名・新API・新画面・ルーティングを確認した。
- この実装ではメール送信、営業先更新、設定変更を行っていない。ログイン済みブラウザの制御ランタイムが利用できなかったため、本番画面のPC/スマホ目視確認は未実施。

## v335 グランピング専用メールのスマホ改行改善

- 本番テストメールと`email_templates!F11`を照合し、文章の途中に入れた固定改行が、スマホ側の自動折り返しと重なって短い行を生んでいたことを特定した。
- 宛名を1行にまとめ、本文は文中の固定改行を廃止して段落間だけ空行を残した。サービス説明とLP URLの区切りだけは意図した1改行として維持し、PCの段落構成を保ちながらスマホでは画面幅に沿って自然に折り返す。
- 本番`email_templates!A11:M11`をversion 2へ更新し、本文変更前の`last_test_sent_at`を空欄へ戻した。専用テンプレートは本番OFF、グランピング優先もOFFのままで、新レイアウトのテスト送信までは開始できない。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`インラインJavaScript構文確認、`git diff --check`が成功した。Apps Script version `344`を作成し、固定Web appを`@344`へ更新した。新レイアウトのメール実送信は行っていない。
...
- 2026年8月15日に固定テスト宛先へ1件送信し、アプリ履歴の成功とGmail送信済みを確認した。送信可能なグランピング施設が0件だったため、優先モードは開始していない。その後v335で本文を変更したため、テスト済み状態は再度解除している。
...
- 確認待ちに営業優先度・連絡先フィルター・優先順ソートを追加し、自動判定の理由を画面上で確認できるようにした。
- 施設名、メール、完全一致ドメイン、親ドメインから既存営業先との関連候補を表示する。親ドメイン一致は自動除外せず、注意候補として扱う。
- 確認済み・送信NGなどの操作履歴を`review_activity_logs`へ保存し、24時間以内かつ後続更新がない操作だけ安全に取り消せるようにした。
- 収集ジョブ一覧に状態・URL・ID検索、最終処理時刻、停止した検索ジョブの続きから再開を追加した。
- 操作履歴の保存に失敗しても営業先の確認結果をエラー扱いに戻さないよう分離し、画面と実データの不一致を防止した。
- `node scripts/smoke-test.js`と`git diff --check`が成功。Apps Script version `335`を作成し、固定Web appを`@335`へ更新した。

## v325 確認操作の画面状態同期

- 添付画面の「確認待ちで選べない更新内容です」はロックエラーではなく、画面内の通常営業リストに残った古い状態を確認待ち専用リストより先に参照し、`未対応`以外を確認操作の前提値として送っていたことが原因だった。
- 確認画面からの操作は選択中の確認待ちデータを直接使い、単件確認では前提状態を常に`未対応`として送るよう修正した。
- サーバー側も画面から受け取った古い前提状態を信用せず、確認操作は実データが`未対応`かをロック内で判定する。実データがすでに変更済みなら従来どおり競合として保護し、上書きしない。
- 単件・一括とも、古い画面状態が混入したケース、再試行、取り消し、実データ側の競合を回帰テストで確認した。`node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`が成功した。
- Apps Script version `334`を作成し、固定Web appを`@334`へ更新した。本番APIでコードv325、対象の「千人風呂 金谷旅館」が`未対応`のまま、未解消エラー0件、実行中ジョブ0件、確認保存待ち0件であることを読み取り確認した。営業先の状態更新や送信は実行していない。

## v324 解消済みエラーの自動整理

- 修正済みの過去エラーを「未解消」から自動的に外し、監査用の「解消済みの履歴」に残すようにした。削除ではなく分類のため、同じ原因が修正後に再発した場合は新規の未解消エラーとして表示される。
- v323以前の確認保存ロック、v322以前のワーカー取得ロック、接続前の観光サイト除外API、改善前の一覧ページ候補未検出、過去の自動メールSheetsタイムアウト、設定時間の入力不備を解消済みとして分類した。
- 自動メールのジョブ取得中にGoogle Sheetsの一時タイムアウトが起きた場合は、0.5秒後・1.5秒後に再試行する。途中で回復すればエラーを残さず、3回とも失敗した場合だけ1件を未解消として記録する。
- 設定値の入力不備はロック取得前に検証し、利用者向けエラーとして返す。入力ミスをアプリ障害ログへ記録せず、他処理とのロック競合も起こさない。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`が成功した。Apps Script version `333`を作成し、固定Web appを`@333`へ更新した。
- 本番APIを読み取り専用で確認し、コードv324、未解消0件、解消済み90件、ストレージ正常、実行中ジョブ0件、確認保存待ち0件だった。営業データ更新、収集、メール送信、フォーム送信、カレンダー招待は実行していない。

## v323 確認済み操作の永続保存待ち

- 本番エラーログを確認し、確認済み操作の失敗7件がすべて`updateReviewLeadDecision`のロックタイムアウトだったと特定した。別処理が長くロックを保持すると、画面側の再試行を使い切って確認待ちへ戻していた。
- 通常の単件・一括確認は、変更された列だけをまとめて書き込み、一覧キャッシュ更新をロック解放後に行うことでロック保持時間を短縮した。
- 5秒以内にロックを取得できない場合もエラーにせず、確認結果をScript Propertiesの永続キューへ保存する。画面更新後も保存待ちの状態を反映し、確認待ちリストへ戻さない。
- 専用の5秒後トリガーで保存待ちを反映し、競合が続く場合は30秒後に再試行する。通常の10分周期バックグラウンド処理にも安全網として同じキュー処理を追加した。
- 同じ営業先で確認と取り消しが続いた場合は、個別のリクエストIDで最新操作を優先し、古い処理が後から上書きしないようにした。
- 単件、一括、取り消し、画面再読込、競合時のキュー保持・再試行を回帰テストで確認した。`node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`が成功した。
- Apps Script version `332`を作成し、固定Web appを`@332`へ更新した。本番確認ではコードv323、保存待ち0件、実行中0件、停止中0件、ワーカー待機中、スキーマ7 / 7正常だった。
- 本番営業先の状態変更、メール送信、フォーム送信、カレンダー招待は検証では実行していない。

## v322 バックグラウンド処理のタイムアウト耐性

- 本番履歴を読み取り専用で調査し、`resort-glamping.com`は完了まで16回、過去の大型収集は180回・526回に分割実行されていたこと、2026-08-02にはロック競合が8件記録されていたことを確認した。調査時点で実行中・停止中ジョブは0件だった。
- 1回の処理上限を300秒から240秒へ短縮し、Apps Scriptの6分制限より前に状態保存・ロック解放・次回予約を行える余白を確保した。本番設定`batch_runtime_budget_ms`も240,000へ変更した。
- 未完了ジョブは5秒後、一時エラーは60秒後、別ワーカーの実行中は30秒後に単発トリガーを再予約し、10分間隔の定期処理だけに依存しないようにした。
- ユーザー処理が残る間は、データ品質移行とダッシュボード集計を後回しにして収集・送信・返信確認の実行時間を優先する。
- 収集中の細かな進捗保存では全キャッシュ削除を行わず、ロックを1秒だけ試す軽量な保存へ変更した。完了・中断時は従来どおり確実に最終状態とキャッシュを更新する。
- 想定内のワーカー取得競合はアプリ障害として記録せず、予期しないワーカー停止時には60秒後の復旧トリガーを予約する。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`が成功した。Apps Script version `330`を作成し、固定Web appを`@330`へ更新した。
- 本番確認ではコードv322、スキーマ7 / 7正常、ワーカー待機中、実行中0件、停止中0件、定期トリガー4本の重複なしを確認した。本番収集、メール送信、フォーム送信、カレンダー招待は検証では実行していない。

## v305 送信NGドメインの収集除外

- 送信NGの営業先に保存されたWEBサイト、問い合わせフォーム、メールアドレスからドメインを抽出し、既存の除外ドメインと統合して収集前に照合する。
- 一致するドメインとそのサブドメインは、施設名やURLのパスが変更されても取得候補から除外する。
- 送信NG以外の営業先は対象にせず、送信NGを解除した場合は`leads`更新時にキャッシュを破棄して再取得可能にする。
- 同じドメインを複数の送信NG営業先が持つ場合は1件へ統合し、5分キャッシュで全件収集時のスプレッドシート再読込を抑える。
- 手動の除外ドメイン、送信NGのWEB・フォーム・メール、サブドメイン一致、通常ドメインの非除外、キャッシュ更新を回帰テストで確認した。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`、`clasp push`、Apps Script version `311`作成、固定Web app `@311`更新が成功した。
- 本番収集、営業データ更新、メール送信、フォーム送信、カレンダー招待は検証では実行していない。

## v304 resort-glamping.com全件収集

- 本番`search_jobs`を読み取り専用で確認し、最新ジョブは停止ではなく`crawl_all:false`・`results_per_query:10`で作成され、10 / 10件を正常完了していたと特定した。
- `resort-glamping.com`は画面の保存設定やチェック状態にかかわらず全件調査へ正規化し、500件を上限として11件目以降もバックグラウンド処理を継続する。
- 画面でも対象URLを入力すると「ページ内の掲載先をすべて調べる」を自動で有効化し、保存済みの旧設定が10件制限でも全件調査へ切り替える。
- 完了済みの10件ジョブは再利用せず、再度「収集を開始」したときに全件調査の新規ジョブを作成する。
- 25施設のサイトマップ回帰テストで10件を超えて候補を保持すること、通常の一覧URLは従来どおり任意の件数設定を使うことを確認した。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`、`clasp push`、Apps Script version `310`作成、固定Web app `@310`更新が成功した。
- 本番スプレッドシートの営業データ更新、メール送信、フォーム送信、カレンダー招待は検証では実行していない。

## v303 Apps Script URL解決・施設取得進捗

- 本番`search_jobs`と`search_results`を読み取り専用で確認し、修正前の最新ジョブは10 / 10件を処理完了して施設名も`ブルードーム京都天橋立`など正しく保存していた一方、2件が既存重複、8件は公式URL未確定となり営業リスト追加0件だったと特定した。
- 原因はNodeのテスト環境に存在する`URL`オブジェクトがApps Script実行環境では使えず、施設詳細ページの絶対・相対リンク解決が空になっていたこと。Apps Scriptだけで動くURL解析へ置き換えた。
- サイトマップ由来の施設IDを`resort_glamping:blue-dome`のような固定IDで保存し、施設詳細取得後は進捗にも仮名`収集対象サイト`ではなく実際の施設名を反映する。
- 実ページHTMLをApps Script相当の`URL`オブジェクトなし環境で検証し、施設名`ブルードーム京都天橋立`、公式URL`https://www.dome-blue.com/`、施設ID`resort_glamping:blue-dome`を取得した。
- `node scripts/smoke-test.js`、全`.gs`構文確認、実ページ詳細抽出、`git diff --check`、`clasp push`、Apps Script version `309`作成、固定Web app `@309`更新が成功した。
- 本番スプレッドシートの営業データ更新、メール送信、フォーム送信、カレンダー招待は検証では実行していない。

## v302 追加URLの重複統合・resort-glamping.com施設名取得

- 本番`search_jobs`と`search_results`を読み取り専用で確認し、v300以降の2ジョブは施設一覧URLへ正しく変換できていた一方、Apps Scriptの取得本文から施設カードを抽出できず、いずれも候補0件で失敗していたと特定した。
- `www`の有無、トップページ、`/accommodation/`の表記違いを`resort_glamping`という同じサイト識別子へ統合し、追加したURLと過去の進捗に同一サイトを重複表示しない。
- 施設一覧HTMLから抽出できない場合は、公式WordPressサイトマップへ自動的に切り替える。実データで403件の施設URLを取得し、各詳細ページから施設名と公式サイトURLを確定する。
- 実ページを使った検証で403件、施設名`ブルードーム京都天橋立`、公式URL`https://www.dome-blue.com/`を取得し、3種類のURL表記が同じ識別子になることを確認した。
- `node scripts/smoke-test.js`、全`.gs`構文確認、実サイトマップ・詳細ページ抽出確認、`git diff --check`、`clasp push`、Apps Script version `308`作成、固定Web app `@308`更新が成功した。
- 営業データを変更する本番収集、メール送信、フォーム送信、カレンダー招待はこの検証では実行していない。

## v301 過去の一覧ページ進捗を常時表示

- 添付画面と現行CSSを照合し、一覧ページ収集の集中表示モードが`.collection-saved-sources`を`display:none`で強制的に隠していたため、ナップを含む過去URL進捗が表示されていなかったと特定した。
- 過去URL欄を非表示対象から外し、「追加したURLと進捗」を現在の収集進捗より上へ移動した。初期状態で開くため、ナップの5,872 / 5,872件・100%をすぐ確認できる。
- 対象サイト単位の直近履歴と、一覧ページURL全体の過去進捗を混同しないよう、空表示の案内文を修正した。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`、`clasp push`、Apps Script version `307`作成、固定Web app `@307`更新が成功した。
- 外部サイト収集、営業データ更新、メール送信、フォーム送信、カレンダー招待はこの検証では実行していない。

## v300 resort-glamping.comの施設収集

- 本番`search_jobs`と`search_results`を読み取り専用で確認したところ、`resort-glamping.com`の4回の実行は停止しておらず、施設候補0件のまま入力URL1件を処理済みとして完了していた。
- トップページURLから実際の施設一覧`https://www.resort-glamping.com/accommodation/`へ自動変換する`resort_glamping`専用プリセットを追加した。
- 実際の施設一覧HTMLで403施設を抽出できることを確認し、施設名、住所、詳細ページURL、重複判定用IDを取得する。
- 全件収集の上限を200件から500件へ拡張し、現在の掲載施設を1ジョブで対象にできるようにした。
- 施設候補0件ではジョブを完了扱いにせず失敗理由を保存し、過去の誤完了ジョブも「処理済み1件・100%」ではなく「候補未検出・0件」と表示する。
- `node scripts/smoke-test.js`、全`.gs`構文確認、実ページHTML抽出確認、`git diff --check`、`clasp push`、Apps Script version `306`作成、固定Web app `@306`更新が成功した。
- 実際の施設収集、営業データ更新、メール送信、フォーム送信、カレンダー招待は検証では実行していない。

## v299 一覧ページ収集の継続処理と全URL進捗

- 1件処理後に止まりやすい経路を見直し、画面からの処理と5秒後の単発トリガーの実行枠を最大300秒へ延長した。
- 対象サイトを処理する前後に毎回行っていた進捗保存を減らし、各対象の完了時を中心に記録することでスプレッドシート書き込み負荷を軽減した。
- 保存設定に残る最大30件だけでなく、過去の`source_page`ジョブ履歴から追加済みURLを復元し、一覧ページ収集画面へ全件表示する。
- 完了済みURLは「全件調査完了」と100%を表示する。調査中、失敗、未実行も同じ一覧で確認でき、進行中は約8秒ごとに更新する。
- 「追加したURLと進捗」を初期状態で開き、過去に追加したURLの総数と各URLの処理件数・割合・最終更新を確認できるようにした。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`、`clasp push`、Apps Script version `305`作成、固定Web app `@305`更新が成功した。
- 匿名アクセスはGoogleログインへ転送されるため、認証済み本番画面の実データ進捗は自動検証していない。外部サイト収集、営業データ更新、メール送信、フォーム送信、カレンダー招待もこの検証では実行していない。

## v298 一覧ページ収集の進捗自動再開

- v297では収集ジョブ登録後の処理を10分間隔の定期トリガーへ任せていたため、待機中のまま進捗が長時間変わらないことがあった。
- 一覧ページ収集画面を開いている間は、待機中ジョブを画面から直ちに1チャンク処理する。処理後も待機中なら次回の進捗更新で続ける。
- 画面を閉じた場合も、5秒後の単発トリガーを作成し、未完了ジョブは単発トリガーをつないで継続する。従来の10分間隔トリガーは復旧用として残す。
- 7分以上更新がない実行中ジョブは「自動再開待ち」として検出し、画面からバックグラウンド修復処理を実行する。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`、`clasp push`、Apps Script version `304`作成、固定Web app `@304`更新が成功した。
- 外部サイト収集、営業データ更新、メール送信、フォーム送信、カレンダー招待はこの検証では実行していない。

## v297 一覧ページ収集の開始エラー修正

- v296で画面から呼び出していた`startSearchJob`とApps Script側の公開関数名が一致しておらず、`[name] is not a function`で収集を開始できなかった。
- Apps Scriptから直接呼び出せる`startSearchJob`を追加し、内部の`startSerperSearchJob`へ安全に接続した。Web API経由でも同じ公開関数を利用する。
- `resort-glamping.com`のようなスキーム省略入力は従来どおり`https://`付きへ正規化されるため、今回の原因はURL形式ではない。
- 公開関数経由で新規ジョブ作成と実行中ジョブの再利用が動作する回帰テスト、全`.gs`構文確認、`git diff --check`、`clasp push`、Apps Script version `303`作成、固定Web app `@303`更新が成功した。
- 実際の外部サイト収集、営業データ更新、メール送信、フォーム送信、カレンダー招待は実行していない。

## v296 一覧ページ収集の対象サイト進捗

- 一覧ページ収集画面へ「収集対象サイトの進捗」を常時表示し、処理済み件数、営業リスト追加、登録済み、対象外・確認要の内訳を確認できるようにした。
- 現在確認中の施設・企業サイト名とURL、直近5件の結果（追加、登録済み、対象外、公式サイト未確認、エラー）を表示する。
- 一覧ページ収集は長い初回処理を画面で待たず、ジョブ登録後すぐ進捗画面へ戻る。収集中は約8秒ごとに進捗部分だけを更新し、入力中の条件を消さない。
- `search_jobs.progress_json`へ対象サイト単位の進捗を保存し、バックグラウンド画面の進捗表示にも反映する。過去ジョブは概要を表示し、対象サイト単位の履歴はv296以降の収集から記録する。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`、`clasp push`、Apps Script version `302`作成、固定Web app `@302`更新が成功した。
- 外部サイト収集、営業データ更新、メール送信、フォーム送信、カレンダー招待は実行していない。

## v295 リンク切れサイトの確認待ち除外

- 今後の収集では、HTTP 404・410に加えてDNS・ホスト解決失敗、無効URL、SSL証明書エラー、接続拒否を明確なリンク切れとして判定し、営業先を作成せず確認待ちに追加しない。
- 既存候補は保存済みの取得エラーを使って判定し、明確なリンク切れが記録されている候補を確認待ち・メール送信・フォーム送信・カレンダー招待の対象から外す。営業データ自体は削除しない。
- HTTP 403・429、HTTP 5xx、タイムアウトはアクセス制限や一時障害の可能性があるため、自動では除外しない。保存済みエラーがない既存候補は、今後の収集・再確認時から判定する。
- 除外した新規候補は`source_page_broken_link`、`dismissed`として監査用の検索結果だけを残す。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`、`clasp push`、Apps Script version `300`作成、固定Web app `@300`更新が成功した。
- 営業データの更新・削除、外部検索、メール送信、フォーム送信、カレンダー招待は実行していない。

## v294 既存の閉鎖候補を確認・送信対象から除外

- v293以前に登録された営業先でも、会社名・施設名に閉鎖、閉店、閉館、閉園、廃業、営業終了等が明示されている場合は確認待ちに含めず、対応不要相当として分類する。
- 閉鎖表記のある既存データは、メール送信、フォーム送信、カレンダー招待の対象にも含めない。元データは削除せず履歴として保持する。
- 「旧店舗は閉店したが新店舗へ移転して営業中」のような継続営業表記は除外しない。
- スクリーンショットの`【R8/3現在 閉鎖】伊豆キャンパーズヴィレッジ`を回帰ケースとして追加し、確認待ち判定、対応不要分類、各送信ブロックを検証した。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`、`clasp push`、Apps Script version `298`作成、固定Web app `@298`更新が成功した。
- 営業データ自体の更新・削除、メール送信、フォーム送信、カレンダー招待は実行していない。

## v293 閉鎖サイトの確認待ち除外

- 一覧ページ収集で公式サイトを確認した際、HTTP 404・410、閉鎖・閉店・営業終了の明示、サイト停止・ドメイン販売ページを検出した候補は営業先を作成しない。
- 閉鎖候補は`source_page_closed`、`dismissed`として監査用の検索結果だけを残し、確認待ちリストには表示しない。通常検索の結果も、閉鎖・営業終了が明示された候補は確認待ちへ登録しない。
- 「旧店舗は閉店したが、新店舗へ移転して営業中」のような継続営業表記は閉鎖判定から除外する。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`git diff --check`、`clasp push`、Apps Script version `297`作成、固定Web app `@297`更新が成功した。
- 外部検索、メール送信、フォーム送信、営業データ更新は実行していない。

## v292 全画面実機監査・読み込み状態・業務文言

- Chromeでログイン済みの固定Web appを開き、主要20画面とバックグラウンド実行状態を実データで撮影・確認した。全画面証跡は`docs/design-audit/v291-live/`、v292の重点再確認は`docs/design-audit/v292-live/`へ保存した。
- 一覧ページ収集は選択済み参考画像と同じく、戻る、タイトル、ソース選択、URL入力、条件、プレビュー、実行操作の順に表示する。参考画像と本番を横並びにした`docs/design-audit/v292-live/07-reference-vs-v292.jpg`で差分を再確認した。
- 初回の遅延取得中に本文が空白に見えていた画面へ、画面固有の読込中表示と`aria-busy`を追加した。メール送信対象で読込案内が即時表示され、その後1,924件の実データへ切り替わることを本番で確認した。
- データ取り込み画面から`doPost`、`fill_blanks`等の内部用語を除去し、上書き、重複判定、バックグラウンド継続の業務ルールへ変更した。固定戻るボタンによる本文の遮蔽も解消し、運用状況にはヘッダー内の戻り先を追加した。
- 送信NGフォームへ入力例、いずれか1項目の必須条件、送信前の無効状態、成功/失敗メッセージを追加した。保存後は全データではなくマスター情報だけを再取得する。
- 本番で`Version v292`、汎用一覧ページ文言、HTML欠損なし、`isOverview`エラーなし、読み込み完了、内部用語なし、固定重なりなしを確認した。
- `node scripts/smoke-test.js`、全`.gs`構文確認、`Index.html`内JavaScriptコンパイル、`git diff --check`、`clasp status`、`clasp push`、Apps Script version `295`作成、固定Web app `@295`更新が成功した。
- 外部検索、メール送信、フォーム送信、営業データ更新、送信NG登録は実行していない。

## v291 初期起動と画面読込の共通復旧

- 初期データの取得に失敗した場合はアプリの読込完了前であるため、v290の画面別再読込だけでは何も実行されなかった。起動前は`refreshAll()`、起動後は対象画面の遅延読込を実行するよう再試行を分岐した。
- 認可が必要な場合は従来どおりGoogle認可ゲートを表示し、それ以外の起動失敗では現在画面の見出し直下へ永続的な再試行案内を出す。これにより、初期通信失敗時もページ更新に頼らず復旧できる。
- 起動失敗が画面内案内へ接続されていることと、起動前の再試行が初期データ全体を再取得することを回帰検査へ追加した。
- `node scripts/smoke-test.js`、`node --check scripts/smoke-test.js`、`Index.html`内JavaScriptコンパイル、`git diff --check`、`clasp push`が成功。固定Web app URLを`@294`へ更新した。
- 選択済みChromeによるログイン済み実画面確認は、Chrome接続機能が現プロセスに読み込まれていないため未実施。外部検索、メール送信、フォーム送信、営業データ更新は行っていない。

## v290 画面データ読込エラーの復旧導線

- 遅延読込に失敗した場合、従来は上部の一時的なエラー文と開発者向けコンソール警告だけで、ユーザーが同じ画面から再試行できなかった。各画面の見出し直下へ、原因文と「再読み込み」を備えた案内を表示するよう統一した。
- 再読み込みは対象画面だけを再実行し、成功時は案内を除去、再失敗時は最新の原因へ更新する。案内は`role="alert"`を持ち、狭い画面では操作ボタンを全幅へ切り替える。
- 全20画面に明示的な遅延読込・描画分岐があることを回帰テストへ追加した。画面内エラーのDOM生成、読み上げ属性、対象画面名を保った再試行操作も単体検証した。
- `node scripts/smoke-test.js`、`node --check scripts/smoke-test.js`、`Index.html`内JavaScriptコンパイル、`git diff --check`、`clasp push`が成功。固定Web app URLを`@293`へ更新した。
- 選択済みChromeによるログイン済み実画面確認は、Chrome接続機能が現プロセスに読み込まれていないため未実施。外部検索、メール送信、フォーム送信、営業データ更新は行っていない。

## v289 固有文言の空状態・クライアント実行契約

- 空状態の案内は共通文言の「データがありません」「営業リストはありません」だけを対象にしていたため、独自に「除外ドメインはまだ登録されていません」と表示する除外画面では、v288の専用案内が適用されなかった。テーブルごとの案内設定がある場合は元の空文言にかかわらず適用するよう修正した。
- 全案内設定が実在するテーブルIDと既存アイコンを参照していることを検査し、除外画面の固有文言と動的`tbody`挿入の両方をDOM回帰テストへ追加した。
- `Index.html`のクライアントスクリプト全体を最小DOM環境で評価し、主要画面描画、画面遷移、遅延読込の12関数がアプリケーションスコープで呼び出せることを自動検査へ追加した。単なる構文解析では見逃す関数の入れ子・初期化時例外を検出できる。
- `node scripts/smoke-test.js`、`node --check scripts/smoke-test.js`、`Index.html`内JavaScriptコンパイル、`git diff --check`、`clasp push`が成功。固定Web app URLを`@292`へ更新した。
- 選択済みChromeによるログイン済み実画面確認は、Chrome接続機能が現プロセスに読み込まれていないため未実施。外部検索、メール送信、フォーム送信、営業データ更新は行っていない。

## v288 動的な空状態の反映・主要一覧の案内拡張

- 空状態の案内処理は`table`自身を受け取った場合しか動かず、実際の再描画でMutationObserverが受け取る`thead`や`tbody`から親テーブルへ辿れなかった。このため、v287で用意した案内が初期化後の一覧に反映されない経路があった。
- 追加された要素から最寄りの`table`を解決して案内を適用するよう修正し、直接子の空状態ラベルだけを対象にして再変換を防いだ。`tbody`挿入を再現するDOM回帰テストも追加した。
- 収集結果、検索利用、バックグラウンド処理、送信準備、送信履歴、同期、商談、送信NG、除外サイト、日別/月別/テンプレート別/分類別分析まで、主要一覧の空状態を業務に合う説明と次操作へ拡張した。
- `node scripts/smoke-test.js`、`node --check scripts/smoke-test.js`、`Index.html`内JavaScriptコンパイル、`git diff --check`、`clasp push`が成功。固定Web app URLを`@291`へ更新した。
- 現在のCodexプロセスはChrome連携設定の変更前から起動したままで、選択済みChromeによるログイン済み実画面確認は次の完全再起動後に行う。外部検索、メール送信、フォーム送信、営業データ更新は行っていない。

## v287 空状態の次操作・動的アイコン描画

- 主要一覧の空状態を、単なる「データがありません」から、状態の意味・次に行う操作・直接移動ボタンを備えた案内へ統一した。確認待ち、営業リスト、テンプレート、フォーム送信、メール送信、実行ジョブ、送信履歴、エラー詳細を対象にしている。
- 後から描画される確認画面の主要ボタンにアイコン定義があっても、初回の静的DOMにしかアイコン変換を適用していなかったため、再描画後は空のアイコン枠になっていた。追加されたDOM自身と配下の両方へ変換を適用するよう修正した。
- 空状態の案内は読み上げ可能なステータスとして扱い、狭い画面では説明と操作ボタンを縦に並べる。
- `node scripts/smoke-test.js`、動的アイコンと空状態のDOM単体回帰、`node --check scripts/smoke-test.js`、`git diff --check`、`clasp push`が成功。固定Web app URLを`@290`へ更新した。
- 選択済みChromeの接続はCodex起動後に設定修正された旧プロセスのため、この版の本番実画面確認は再起動後に実施する。外部検索、メール送信、フォーム送信、営業データ更新は行っていない。

## v286 動的入力・画面遷移・タスクセンターのアクセシビリティ

- 全20画面について、画面セクション、`TAB_TITLES`、業務説明、表示見出し、356個のインライン操作を対応付けた。20画面すべてにページ名・短い目的説明・`h1`があり、すべての操作が実在するクライアント関数または標準ブラウザ操作へ解決することを自動検査へ追加した。
- 静的な84ボタン、2リンク、51入力部品は識別名を持っていたが、動的に生成する検索結果のメール/フォーム編集、ジャンル名/説明、理由名/説明、テスト送信先に読み上げ名が不足していた。各入力へ内容に合う`aria-label`を追加し、全114入力・33選択・12テキスト領域で未解決ラベル0件を確認した。
- SPA内で画面を切り替えてもキーボードフォーカスがサイドバーに残り、新しい画面が読み上げられない問題を修正した。遷移後は対象画面の`h1`へフォーカスし、キーボード操作時は明確なフォーカス枠を表示する。
- タスクセンターを開いた時は閉じるボタンへ移動し、データ再描画後もフォーカスを維持する。Escapeまたは閉じる操作では起点へ戻し、タスクセンター内から別画面へ進む場合は遷移先見出しへ移す。
- 成功/注意の基本色を既存の高コントラスト色へ統一し、一覧ページURLのプレースホルダーを本文用のミュート色へ変更した。入力前の例示を保ったまま白背景上の可読性を上げた。
- `node scripts/smoke-test.js`、`Index.html`内JavaScriptコンパイル、全画面DOM契約、入力ラベル、インライン操作関数、API整合性、`git diff --check`、`clasp push`が成功。固定Web app URLを`@289`へ更新した。
- 選択済みChromeの制御ツールは今回も公開されていないため、Product Design監査が要求する現在実装のログイン済み20画面撮影・画像比較は未実施。外部検索、メール送信、フォーム送信、営業データ更新は行っていない。

## v285 共通ダイアログ操作・DOM整合性・実行時間警告

- 全画面の静的アクセシビリティ監査で、静的ボタン84件とリンク2件には識別名がある一方、設定検索1件だけラベルがなく、3種類の動的ダイアログと営業先詳細ダイアログはフォーカス管理が不足していた。設定検索へ`aria-label`を追加し、ダイアログの役割を背景ではなく実パネルへ移して見出しと関連付けた。
- 共通ダイアログ制御を追加し、開いた時のフォーカス移動、Tab/Shift+Tabの閉じ込め、Escapeで閉じる操作、閉じた後の起点への復帰、表示中の背景スクロール停止を、営業先詳細・テンプレート編集・テンプレートテスト・自動送信確認へ統一した。
- 一覧ページ収集の2択は、矢印キー未実装の不完全なARIAタブではなく、実際の動作に合う押下状態付きボタングループへ変更した。存在しない`gmailStatusPills`を参照する死んだ分岐と未使用変数も削除し、文字列で参照するDOM IDがすべて静的定義へ解決する回帰検査を追加した。
- 成功色と注意色を白背景で読みやすい既存の濃色トークンへ統一し、残っていた英語の補助見出し`Collection activity`と`Send check`も日本語化した。
- バッチ実行時間は330秒へ丸めた後に330秒超過を判定していたため、警告が絶対に発火しなかった。設定値と有効値を分離し、360秒等の危険な設定は有効値を330秒へ制限しつつ警告へ残すよう修正した。
- `node scripts/smoke-test.js`、`Index.html`内JavaScript構文確認、文字列DOM参照整合性、`git diff --check`、`clasp push`が成功。固定Web app URLを`@288`へ更新した。匿名GETはGoogleログインへ正常に到達したが、このセッションでは選択済みChromeの制御機能と`clasp run`実行権限がないため、ログイン済みHTML/APIのv285応答と全20画面の画像比較は未実施。
- 外部検索、メール送信、フォーム送信、営業データ更新は行っていない。直前の認証付きv284監査ではスキーマ7/7、ワーカー/ストレージ正常、未解決エラー0件を確認済みであり、v285はUI共通処理と読取専用の利用状況計算だけを変更している。

## v284 全画面UI・API・運用状態の整合性監査

- 20画面、356個のインライン操作バインディング、68個のクライアントAPIアクションを静的に棚卸しし、未配線API、未対応のグローバルメッセージ色、重複関数、生の`https://`によるHTML欠損がないことを確認した。`getLeadListStats`だけWeb API経路が欠けていたため、`doPost`のディスパッチへ追加した。
- 主要画面の英語補助見出しと曖昧な画面名を日本語の業務目的へ統一した。「営業フロー・ステップボード」は「今日の営業フロー」、「メールテンプレート」はフォーム営業も含む「営業テンプレート」、「稼働分析」は「成果・分析」、「営業リスト同期」は「営業リスト取り込み」、「管理/運用」は「運用状況」とし、ナビゲーションとブラウザタイトルも同じ名称へ揃えた。
- GAS利用状況がコード名の`v284`を保存済みApps Scriptバージョン数と誤認して200版上限の危険警告を出していた。コード版は情報Pillとして表示し、クォーターメーターと警告から除外した。旧キャッシュに残ったバージョン警告も読取時に除去し、現在のコード版へ正規化する。
- `getAppInfo()`は`SPREADSHEET_ID`が保存済みでもキャッシュ切れのたびにSpreadsheetを開いていた。保存済みIDからURLとアプリ情報を返す高速経路へ変更し、DB作成が必要な初回だけSpreadsheetを開く。
- 本番スキーマの`raw_import`に不足していた`source_row_number`と`result_json`を安全な`setup()`で追加し、7/7チェックが正常になった。バックグラウンドワーカーとストレージは正常、未解決エラーは0件。過去のGoogle Sheets一時接続障害と今回の監査用不正引数ログは時間と内容を限定して解消済み表示にしたため、将来同じエラーが再発した場合は未解決として残る。
- 正しい引数で24本の本番読取ルートを再検証し、すべて成功した。固定URLの認証付きHTMLはHTTP 200で、新しい5画面名と一覧ページ収集ワークベンチの連続したマークアップを確認した。`getInitialData`は約3.3秒、スキーマ7/7、ワーカー/ストレージ正常、誤ったバージョン警告なし、未解決エラー0件だった。
- `node scripts/smoke-test.js`、`Index.html`内JavaScript構文確認、クライアントAPI/サーバールート整合性、`git diff --check`、`clasp push`が成功。固定Web app URLを`@287`へ更新し、認証付き`getAppInfo`で`20260722_apps_script_full_workflow_v284_app_wide_audit_cleanup`を確認した。
- 選択されたCodex Browser制御ランタイムはこのセッションで公開されていないため、ログイン済み全20画面の同一ビューポート画像比較は未実施。ユーザー提供済みの収集画面と参考画像はv281で反映済みであり、今回は配信HTML、UI構造、API、実データを変更しない読取経路を検証した。外部検索、メール送信、フォーム送信、営業データ更新は行っていない。

## v281 参考画像準拠の一覧ページ収集画面

- ユーザー提供の確定デザインを`docs/design-references/collection-source-reference-v281.png`へ保存し、一覧ページ収集ルートでは送信ステータス、親見出し、収集ステータス、その他の取り込み方法、保存済みURL進捗を初期表示から外した。
- 戻る、画面タイトル、ソース選択、単一URL入力、条件サマリー、条件変更、収集内容プレビュー、保存/開始ボタンの順に再配置し、幅、余白、文字サイズ、境界線、線アイコン、操作ボタンを参考画像へ合わせた。
- URL欄を`textarea`から閉じタグ不要の単一行`input`へ変更。Apps Script配信時にプレースホルダー内の生の`https://`以降が欠落し、後続HTMLが入力欄へ巻き込まれていたため、`HTTPS_PROTOCOL_PREFIX`を実行時に挿入する方式へ変更した。
- `Index.html`に生の`https://`を再導入しないテスト、URL入力が`textarea`へ戻らないテスト、集中表示クラスと条件ブロックの存在テストを追加した。
- `node scripts/smoke-test.js`、`Index.html`内JavaScript構文確認、`git diff --check`、`clasp push`が成功。固定Web app URLを`@284`へ更新し、認証付き`getAppInfo`で`20260722_apps_script_full_workflow_v281_source_page_reference_fidelity_hotfix`、認証付き配信HTMLでURL入力から確認ボタン、条件ブロックまで欠損なく連続していることを確認した。
- 外部検索、メール送信、フォーム送信、営業データ更新は検証中に実行していない。

## v279 収集画面の描画停止修正

- 本番v278で全ページ上部に`isOverview is not defined`が表示され、収集画面の本文が空になることをユーザー提供Chrome画面で確認した。
- 原因は、一覧ページ収集のプレビュー再計算処理が`renderCollectionCommandCenter`ではなく、無関係な`renderSearchActivityPanel`へ挿入されていたこと。そこで収集画面だけに存在する`isOverview`と`activeTab`を共通更新時に参照し、`ReferenceError`で描画処理を中断していた。
- 再計算処理を両変数が定義される`renderCollectionCommandCenter`の末尾へ移動した。検索アクティビティ描画と結果レビュー描画が収集専用変数を参照しないことも回帰テストで固定した。
- `node scripts/smoke-test.js`、`Index.html`内JavaScript構文確認、`git diff --check`、`clasp push`が成功。固定Web app URLを`@282`へ更新し、認証付き`getAppInfo`で`20260722_apps_script_full_workflow_v279_collection_render_scope_fix`を確認した。
- 外部検索、メール送信、フォーム送信、営業データ更新は検証中に実行していない。

## v278 汎用一覧ページ収集ワークベンチ

- ユーザー選択デザインを`docs/design-references/collection-universal-v278.png`へ保存し、既存のネイビーサイドバー、青白の配色、線アイコン、ボタン階層を維持した。
- なっぷやキャンプ専用の入力支援を画面から外し、会員一覧・企業一覧・店舗一覧・施設一覧・出展企業一覧など任意の一覧ページURLに対応する構成へ変更した。
- 「新しい一覧ページURL」と「保存済みURLを使う」を最初に選び、URL入力、現在の条件、変更可能な詳細条件、収集内容のプレビュー、保存/開始の順で迷わず進める。
- 分類は「未分類」のまま利用可能。処理件数、ページ内全件、検索補完、保存名を任意で調整でき、保存済みURLには条件も保持する。
- 汎用ページ抽出を回帰テストし、内部詳細リンクと外部公式サイト候補を識別できることを確認。全件モードでは最大200候補をカーソル付きバックグラウンド処理へ渡す。
- `node scripts/smoke-test.js`、`Index.html`内JavaScript構文確認、操作ID一意性確認、`git diff --check`、`clasp push`が成功。固定Web app URLを`@281`へ更新した。
- 認証付き`getAppInfo`で`20260722_apps_script_full_workflow_v278_universal_source_collection`、認証付きHTML取得でHTTP 200と6個の汎用UIマーカーを確認した。
- 選択されたCodex Browserの制御機能がこのセッションでは公開されていないため、同一ビューポートの実画面比較は未実施。外部検索、メール送信、フォーム送信、営業データ更新は行っていない。

## v277 営業リスト収集画面の集中レイアウト

- 提供画像と画面コード全体を監査し、URL入力より上に状態カード、方法変更、その他の方法、重複見出しが並び、実行ボタンが初期表示外になる問題を特定した。監査画像と詳細は`docs/audits/v277-collection-focus/`へ保存した。
- 選択中の方法は小さな現在地バーへ変更し、フォームを直下へ移動。状態表示とその他の取り込み方法はフォーム後へ下げた。
- PCでは入力・条件を左、確認・実行を右に並べ、実行カードをスクロール中も確認できる。980px以下は1列、620px以下は状態表示も1列へ戻す。
- 既存の入力ID、保存、収集、結果、確認待ち導線は変更せず、方法切替後のフォームフォーカスだけを追加した。
- `node scripts/smoke-test.js`、`Index.html`内JavaScript構文確認、`git diff --check`、`clasp push`が成功。固定Web app URLを`@280`へ更新し、認証付き`getAppInfo`でv277、認証付きHTML取得で集中レイアウトのクラス、フォーカス表示、主要実行ボタンを確認した。
- Codex Browserの制御ランタイムがこのセッションでは公開されていないため、デプロイ後の同一ビューポート画像比較は未実施。外部検索、メール送信、フォーム送信、営業データ更新は行っていない。

## v276 営業リスト収集ツールのガイド型UI

- 初期画面に`方法を選ぶ → 条件を入力 → 収集を開始 → 確認して承認`の4手順を明示し、操作後の流れまで最初に理解できる構成へ変更した。
- 収集方法は、内部機能名ではなく「業種・地域から探す」「一覧ページから取り込む」の2択へ整理。各カードに必要な入力情報を表示した。
- キーワード収集は、必須条件、収集量、実行前確認の3カードへ分割。補助語は任意設定へ移動し、既定値のままでも迷わず実行できるようにした。
- 一覧ページ収集も同じ3段階へ統一し、保存済みURLと進捗は入力フォームから分離して折りたたみ表示にした。
- 実行後は追加・重複・除外件数を結果バナーへ表示し、バックグラウンド進捗と確認待ちへ直接移動できるようにした。
- 自動収集、CSV、除外ドメインは「その他の取り込み方法」へ集約し、実行履歴・検索環境との重複導線を減らした。
- `node scripts/smoke-test.js`、`Index.html`内JavaScript構文確認、`git diff --check`、`clasp push`が成功。固定Web app URLを`@278`へ更新し、認証付き`getAppInfo`でv276、認証付きHTML取得で新しい見出しと主要ボタンを確認した。
- Codex Browserの操作機能がこのセッションでは公開されていないため、デプロイ後のクリック操作と同一ビューポート画像比較は未実施。メール送信、外部検索、営業データ更新は行っていない。
...
## 2026-08-02 v327 操作支援機能

- 確認待ちへ削除を伴わない3分類の自動仕分けを追加。重複、情報・観光・ブログサイト、休業・閉鎖、リンク切れは理由付きの自動除外候補へ集約し、手動復元と自動判定への再適用を可能にした。
- ダッシュボードに優先候補名、停止ジョブ、本日の送信対象を並べる今日の作業リストを追加した。
...
## 2026-07-22 v275 ダッシュボードの営業フロー化

- 旧ダッシュボードで送信・API・月次指標が複数カードへ重複し、最優先の確認待ちが下部へ埋もれていた構造を整理。
- 案2を基準に、`今日 → 収集 → 確認 → 送信 → 成果`の5段階ボードを最上部へ配置し、各工程から既存画面へ直接移動できるよう変更。
- `確認待ち`、`エラー`、`送信キュー`を優先順のタスクキューへ統合。API・送信時間・本日送信済みは右側の運用状況へ集約。
- 今月の新規リスト・返信・商談だけを成果サマリーとして残し、送信制御・収集状況・詳細内訳は折りたたみへ移動。
- 既存ナビゲーション、アイコン、配色、安全ステータス、送信制御、集計表は削除せず再配置。
- 900px以下では主要2列を1列化し、620px以下ではタスク操作と成果カードを縦積みにするレスポンシブ対応を追加。
- 自動テスト、インラインJavaScript構文、静的差分検査は通過。Codex Browser実行機能が利用できないため、同一ビューポートの最終画像比較は未完了。
- 既存Web app URLを固定したままdeployment `@277`へ更新。メール送信・検索実行・営業データ更新は行っていない。
