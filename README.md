...
- 全21画面の日本語見出しとナビゲーション名称を業務目的に統一し、現在地を画面名・ブラウザタイトル・サイドバーで一致させたUI
...
- Web app deployment @362 / production code v352: `https://script.google.com/macros/s/AKfycbwJcZuTk-7wuFJapBdo4dk-yj64hFHk71BMuJxO-pl9BWpui3kOt17lmPT_7LfnZ0OV-g/exec`
- Apps Script HEAD / repository code v352: `20260818_apps_script_full_workflow_v352_mail_status_next_send_first`
...
- Apps Script Version 113 / code v113でGmail連携/管理の共通ステータスカード内のラベルと詳細を縦積み固定にし、`Google Sheets5,441件` のようにテキストが詰まる崩れを防ぐ余白・折り返し・Pill配置ガードを追加しました
- Apps Script Version 114 / code v114で管理/Gmailの詳細アコーディオン内カードを上揃えにし、子カードが同じ高さに引き伸ばされてボタンやPillの上に大きな空白が出る状態を抑制しました
- Apps Script Version 115 / code v115で管理詳細アコーディオン内の子カードを読みやすい2列ベースへ変更し、4枚目だけが左下に残って右側に大きな空白が出る配置を抑制しました
...
### v352 メール送信状況を「次回送信」中心に再設計

- メール送信状況の情報順を、`自動送信の稼働状態 → 次回送信数 → グランピング優先 → 明日の候補 → 今日の結果`へ整理しました。画面を開いた直後に、送信できる状態かと次に何件送るかを判断できます。
- 4枚の同格な集計カードを廃止し、稼働・次回確認・安全状態・停止操作を1本の運用バーへ集約しました。正常時の説明アラートは表示せず、失敗や送信中など対応が必要な場合だけ警告を出します。
- 明日の候補を画面の主役にし、予定日、候補件数、1日上限、1回上限、進捗、送信時間帯を同じ領域へまとめました。候補表はその直下に置き、施設詳細へ移動できる既存操作を維持しています。
- グランピング優先は候補表の直前へ横長の状態バーとして配置し、対象件数、他ジャンル停止、使用テンプレート、対象内訳、LP確認、解除操作をまとめました。
- PCでは候補表を広く使い、狭い画面では運用バーと次回送信情報を縦並びにします。表は横スクロールを維持し、主要ボタンはスマホ幅で押しやすく折り返します。
- 送信候補の算出、自動送信条件、日次・バッチ上限、優先開始・解除、送信停止などのサーバー処理は変更していません。

### v351 ボタン操作とフォーム候補表示の体感高速化

- 共通API処理は、開始直後に画面全体を処理中へ変えず、450msを超えた処理だけ上部の進捗表示とボタン制御を出します。短時間で終わる保存・読込ではローディング表示が点滅しません。
- メニュー切替の進捗表示を、未取得データがある画面だけに限定しました。取得済み画面への再移動は即時切替になり、従来の固定650ms表示を廃止しました。
- フォーム送信済み、送信不可、送信済み解除は共通ローディングを使わず、対象施設の操作状態とローカルキューだけを更新します。候補読込も画面固有の状態表示だけを使います。
- フォーム候補は同じ検索・ジャンル条件をブラウザで60秒保持し、サーバーでは営業先・テンプレート・送信履歴・NG設定の更新に連動する5分キャッシュを使用します。更新後はrevisionが変わるため、古い候補を継続利用しません。
- フォーム送信メニューへポインターまたはキーボードフォーカスが入った時点で、テンプレートと候補をバックグラウンド先読みします。画面を開いた直後の待ち時間を短縮します。
- 1.2秒を超えたAPI処理はブラウザconsoleへ`[api-performance]`として処理名と時間を記録し、次の遅延診断に利用できます。

### v350 フォーム送信の連続作業キュー

- フォーム送信画面を、一覧表中心から「現在の1施設」と「次の5件」に集中できる2列レイアウトへ変更しました。公式サイトのトップを開いて本文をコピーし、手動送信後に送信済みまたは送信不可を記録する流れを1画面で進められます。
- メインキューには、本番フォームテンプレートが設定済みで、公式サイトがあり、送信条件を満たす施設だけを最大25件表示します。テンプレート不足、送信NG、公式サイト不足、待機期間中などは「要確認」へ理由別に分離します。
- 「送信済み・次へ」と「送信不可・次へ」は確認画面を出さず、表示中の施設だけを更新して次へ進みます。送信不可は営業先を対応不要へ更新し、理由`フォームが見つからない`と時刻を構造化して保存します。
- PCでは`O`で公式サイトを開いて本文コピー、`S`で送信済み・次へ、`X`で送信不可・次へを実行できます。入力欄、モーダル、ボタン操作中にはショートカットを発火させません。
- 25件を終えるまでは一覧を再読込せず、最後の1件を処理した時点で次の最大25件を自動取得します。グランピング専用フォームは、通常フォーム送信から30日以上経過した施設も既存の専用キャンペーン判定に従って候補へ含めます。

### v349 WEBサイトアイコンの巨大化修正

- v348で施設名へ追加した外部リンクSVGに寸法指定がなく、ブラウザ既定サイズまで拡大してフォーム対象の行高が崩れていました。
- 施設名リンクのアイコンを14px、作業中施設名のアイコンを15px、URLミニリンク内のアイコンを14pxへ固定しました。
- WEBサイトを開く機能、屋号コピー、作業対象選択の役割分離は維持し、表の行高だけを通常サイズへ戻します。

### v348 フォーム送信リストのWEBサイト導線

- 表の後半にあったWEBサイト列を廃止し、「営業先・WEBサイト」セルへ公式サイト導線を統合しました。施設名と「WEBサイトを開く」のどちらからでも、新しいタブで公式サイトを開けます。
- 青い施設名が屋号コピーに見える曖昧な挙動をやめ、施設名はWEBサイト、屋号コピーはクリップボードアイコン付きの別ボタン、対象選択は「作業」ボタンへ役割を分けました。
- 作業中バーでは「WEBサイトを開く」を最初の青い主要ボタンにし、施設名自体からも公式サイトを開けます。狭い画面では各操作を折り返して押しやすい幅にします。
- 本番データを読み取り確認し、添付画面で選択されていた施設には`website_url`と`form_url`の両方が保存済みで、データ不足ではなく列位置が主因だったことを確認しました。

### v347 フォーム送信作業の操作性改善

- フォーム送信一覧と作業中バーから営業先のWEBサイトを直接開けるようにし、屋号コピー、本文コピー、フォーム表示までの移動を短縮しました。
- グランピング向けChatGPT広告フォーム本文は、`屋号`と`ご担当者様`を別行で表示します。保存済みの旧テンプレートが残っていても、組み込み最新版を画面表示と送信済み履歴へ適用します。
- 施設選択、屋号コピー、本文コピーは一覧全体を再描画せず、該当要素だけを更新します。一覧の明示更新時も既存行を残したまま件数を「更新中…」へ切り替えます。
- 「送信済み」と「解除」はサーバーが返した更新済み営業先を該当行へ反映し、フォーム一覧と営業先一覧の全件再取得を行いません。検索条件、選択中施設、スクロール位置を維持します。

### v346 フォーム用ChatGPT広告テンプレートのクイックコピー

- フォーム送信リスト上部に「ChatGPT広告用テンプレート」の専用コピーエリアを追加しました。表示中の送信可能なグランピング施設があれば、施設名を差し込んだ本文を1クリックでコピーできます。
- 施設を選択していない場合でも、専用テンプレートの原文をすぐコピーできます。差し込み済み本文と原文を別ボタンにし、何がコピーされるかを明示しています。
- 表内と本文プレビューのコピー操作も、専用テンプレートの場合は「ChatGPT広告をコピー」と表示します。コピー成功後はボタンを緑色の「コピーしました」へ一時的に切り替えます。
- 「グランピングを表示」から既存のジャンル絞り込みを適用できます。コピー操作だけでは外部フォーム送信、送信済み登録、営業先更新を行いません。

### v345 フォーム送信リストのジャンル絞り込み

- フォーム送信リストの検索条件にジャンルを追加し、ジャンルマスターの有効な選択肢から1つ選んで絞り込めるようにしました。
- ジャンル変更時は一覧をすぐ再取得し、検索、フォーム状態、表示件数の条件と組み合わせて利用できます。「クリア」でジャンルも「すべて」へ戻ります。
- 絞り込みはブラウザ内の見た目だけではなく、`listLeads`の`genre`条件としてサーバーへ渡し、件数と一覧を同じ条件で返します。

### v344 メール対象施設のフォーム一覧重複除外

- 同じ営業先レコードに有効なメールがある場合に加え、同一ジャンル・同一施設名の別レコードに有効なメールがある場合も、フォーム送信一覧からフォーム専用側のレコードを除外します。
- 施設名は会社種別、空白、記号を正規化して照合します。短すぎる名前や「キャンプ場」「ホテル」などの汎用名は照合対象にしません。
- まとめサイトなどの共有ドメインだけでは同一施設と判定しません。別施設が同じドメインを使っていてもフォーム一覧から誤って消えないようにしています。

### v343 グランピング向けChatGPT広告のフォーム営業対応

- フォーム送信画面で、グランピング施設には専用の「グランピング向けChatGPT広告（フォーム）」を自動選択します。通常のフォーム用テンプレートが同じジャンルまたは共通で本番ONなら、施設ごとに手動で切り替えられます。
- 専用フォーム文面はChatGPT広告LP `https://adclutch.jp/glamping-chatgpt-ads` を含み、フォーム欄へ貼り付けやすいようメール版より短くしています。アプリが外部フォームを自動送信することはなく、従来どおり本文コピー、外部フォームでの手動送信、送信済み登録の順で運用します。
- 通常フォームを送信済みのグランピング施設も、前回送信から30日以上経過していれば専用フォームの対象へ戻します。30日未満、送信日不明、確認待ち、送信NG、商談中などは理由を画面に表示して停止します。
- 同じChatGPT広告を専用メールで送信済みまたは送信処理中の施設はフォーム送信を止め、専用フォームで送信済みの施設はメール送信を止めます。フォーム送信済みを解除した場合も、それ以前の通常フォーム履歴は保持します。

### v342 メール送信状況の判断優先レイアウト

- 画面上部は「自動送信」「明日の候補」「優先ジャンル」「要確認」の4項目に限定し、状態・予定・優先・異常を最初に判断できるようにしました。
- PCでは明日の候補を左、グランピング優先設定を右へ配置し、今日の送信履歴は確認用として下段へ移動します。1180px以下では主要領域を縦積み、760px以下ではカードを縦積みし、一覧の横スクロールを維持します。
- 優先設定カード内で対象件数、未送信・通常送信済み内訳、30日ルール、専用メール送信済み除外、開始・解除、対象内訳確認を完結できます。
- 候補が0件の場合は、グランピング施設が表示されない理由を最大4項目まで件数カードで表示し、全施設の理由別一覧へ移動できます。

### v341 返信同期とダッシュボード送信候補の整合

- Gmailで返信を検知した営業先がすでに「送信NG」の場合、送信NGフラグとステータスを保持したまま返信確認済みにします。返信検知が送信停止を解除することはありません。
- ダッシュボードの「本日の送信対象」は、単なるメール保有件数ではなく、本番テンプレート、ジャンル、グランピング優先設定、送信履歴、本日の残り上限、トリガーと送信制御を通した実際の自動送信候補数に統一しました。
- 2026-08-16に `gmail_reply_check` を有効化し、Gmailで人からの返信と確認できた3件を過去分として同期しました。3件とも元の送信NG状態は保持されています。

### v340 グランピング専用メールの30日再送対象

- グランピング優先では、メール未送信施設を最初に送り、その後に通常メールの前回成功送信から30日以上経過した施設を送ります。通常メール送信済みでもChatGPT広告専用メールの対象になります。
- `glamping-chatgpt-ads-v1`の成功履歴が営業先IDまたは宛先にある施設は対象外です。専用メールの履歴種別は`グランピングChatGPT広告`として保存し、候補表示と送信直前の両方で重複を止めます。
- 優先開始前の確認、メール送信状況、明日の候補、手動一括送信候補、理由診断で同じ対象判定を使用します。診断には30日待機中や専用メール送信済みも表示します。
- 2026-08-16の本番DB読み取り確認では、グランピング743件のうち候補333件（未送信0件、通常メール送信済み333件）、30日待機中5件、専用メール本番成功0件でした。

### v339 グランピング優先の対象・除外理由表示

- 送信可能なグランピング施設が0件でも優先ボタンを無効化せず、「開始できない理由を確認」として操作できるようにしました。押しても優先設定は変更せず、対象判定パネルを開きます。
- グランピングの全営業先を、送信可能、メールアドレスなし・不正、送信済み、営業対象外、送信NG、確認待ち、除外ドメインなどの理由別に集計し、施設単位の判定理由まで表示します。理由ボタンで絞り込み、施設名から営業先詳細を開けます。
- 本番DBの読み取り専用確認ではグランピング743件、送信可能0件でした。内訳はメールアドレスなし・不正359件、送信済み346件、営業対象外19件、送信NG10件、確認待ち8件、除外ドメイン1件です。

### v338 メール送信状況の余白・テンプレート待ち表示修正

- 送信状況の各パネルへ既存カードと同じ内側余白を適用し、見出し・説明・表の間隔を統一しました。PCの情報密度を保ちながら、スマホのカード縦積みと一覧横スクロールを維持します。
- 明日の実送信候補が0件でも、送信可能だがジャンルに対応する本番テンプレートがない営業先を「本番テンプレート設定待ち」として最大1日上限件数まで表示します。未設定のまま自動送信することはなく、営業先詳細とテンプレート画面を直接開けます。
- 本番DBの読み取り専用監査では、送信可能な固有宛先1,953件の内訳がECサイト1,922件、介護施設29件、ジャンル未設定2件でした。本番ONの初回メールテンプレートはグランピング・温泉旅館・キャンプのため、実際の明日送信候補は0件です。

### v337 メール送信状況の読み込み・レイアウト修正

- 初回表示で約1万件の営業先候補まで一括計算していた処理を、今日の履歴・稼働状態を返す軽量取得と、明日の候補計算へ分割しました。今日の状態を先に表示し、明日の一覧だけを独立して計算します。
- 明日の予定順は、送信可能候補を一度だけ選定し、既存の1回上限ごとのジャンル・テンプレート順へ並べ替える方式に変更しました。候補選定を繰り返さず、実際の送信順を維持します。
- 候補計算中は0件と誤表示せず「計算中」を表示し、取得失敗時は原因と「候補だけ再取得」を表示します。待機カードは全幅表示にし、PCの過度な横伸びを抑え、スマホの縦積み・横スクロールを維持します。

### v336 メール送信状況ボード

- 主要メニューへ独立した「送信状況」画面を追加し、完全自動送信の稼働状態、送信時間、次回送信、本日の残り、グランピング優先、使用テンプレート、停止・警告理由を上部へ集約しました。
- 今日の送信履歴は成功・失敗・送信中・テスト送信を分け、施設、ジャンル、宛先、テンプレート、種別、エラー詳細を表示します。失敗・送信中から再送する操作は置かず、営業先詳細だけを開けます。
- 明日の候補は既存の送信可否・優先・二重送信防止と同じ判定を使用し、1回の上限ごとに候補を再計算して、実際のテンプレート別実行順で1日の上限まで表示します。
- 画面を開いた時と設定操作直後に最新状態を取得し、開いたままの場合は日本時間の6:30・12:00・18:00に自動更新します。管理者だけが自動送信とグランピング優先を操作できます。
- PCでは状態カードと一覧を横に活用し、760px以下では状態カードを縦積み、履歴と候補テーブルを横スクロール表示に切り替えます。

### v335 グランピング専用メールのスマホ改行改善

- グランピング専用メールは、文中の固定改行を廃止し、宛名と段落の区切りだけに改行を使用します。PCでは従来どおり段落単位で読みやすく、スマホでは画面幅に合わせて自然に折り返します。
- 本文変更後は専用テンプレートのテスト済み状態を解除し、再テスト送信が完了するまでグランピング優先を開始できない状態に戻します。
...
- 単件・一括の送信NGは確認画面を開かず、その場で処理して次の候補へ進みます。
- 送信NGにした営業先のドメインは必ず今後の収集対象から外します。保存待ちになった場合も除外指定を維持します。
- 誤操作時に戻せるよう、単件は10秒間の取り消し、一括は最近の確認操作からの取り消しを維持します。
- 「確認済み」の一括処理だけは、対象件数を確認できる従来の事前確認画面を残します。

### v331 確認待ちの高速操作

- 確認待ちで`A`（確認済み）、`N`（送信NG）、`E`（編集）、`J / K`（次／前）を利用でき、確認済み・送信NG後は次の候補へ自動で進みます。入力欄・編集画面・ダイアログ表示中は誤操作を防ぐため無効です。
- 連絡先の絞り込みを、件数付きの「すべて」「メールあり」「フォームのみ」「連絡先なし」へ変更しました。初期表示は全件かつメール取得済み順で、操作・更新後も選択中の条件を維持します。
- 送信NG時は理由を必須選択し、対象件数とドメイン数を確認できます。「今後このドメインを取得しない」は別のチェック項目として初期OFFにし、共有ドメインの誤除外を防ぎます。
- 新しい送信NG判断ではドメイン除外のON/OFFを保存待ちキューにも保持し、取り消し時は除外フラグも戻します。過去データで明示設定がない送信NGは従来の除外挙動を維持します。

### v330 メール取得済み順を全画面へ統一

- 確認待ち専用画面だけでなく、「一括操作／一覧ページ」と一覧の確認待ちフィルターでも、メール取得済みを初期表示の先頭にしました。
- 画面の「メールあり」と同じく、メール欄に値があるかどうかで並び順を判定します。メール送信時の有効性チェックは従来どおり別途行います。
- メール取得済みの中、未取得の中では、それぞれ更新日時の新しい順に表示します。
...
- 確認待ちからドメイン履歴を開き、収集・確認・送信NG・収集除外・送信の記録を横断表示できます。
