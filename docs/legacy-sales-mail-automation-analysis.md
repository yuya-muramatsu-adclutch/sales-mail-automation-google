...
- `app/ng-master/page.tsx`, `app/exclusions/page.tsx`, `components/ExcludedDomainManager.tsx`: 送信NG/除外ドメイン管理、集計、除外ドメインの検索/状態フィルタ/操作列
...
- 送信プレビューの差し込み値カードに空欄タグ一覧、空欄数Pill、警告色カードを追加し、送信前に未入力タグだけを確認できるようにした。
- 送信プレビューに本文差分パネルを追加し、テンプレート原文、送信時本文、空欄タグの前後文脈を並べて確認できるようにした。
- 送信プレビューに件名差分パネルを追加し、テンプレート件名、送信時件名、件名内の空欄タグ文脈を本文と同じ密度で確認できるようにした。
...
- 旧 `background-jobs/page.tsx` のページ冒頭に合わせ、バックグラウンド進捗のヘッダー操作をArrowLeft/ListChecksアイコン付きにし、「進捗の見方」パネルへ旧ページの説明文とListChecksアイコンを追加した。
